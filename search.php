...
<h2 class="display-4">Searching results:</h2>
<div id="app">
    <!-- Modal -->
...
<div class="contact-list" style="padding: 30px;">
    <?php
    // Read the text file and find data
    $content = readRecords();
    if (count($content) > 0) {

        foreach ($content as $c) { 
            if (strpos(strtolower($c['title']), $input) !== false || strpos(strtolower($c['fname']), $input) !== false ||
            strpos(strtolower($c['lname']), $input) !== false || strpos(strtolower($c['email']), $input) !== false) { 
                ?>
